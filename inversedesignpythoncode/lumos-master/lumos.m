...
function [z, p, vis_result] = lumos(name, problem, paradigm, param_type, ...
    ...
    %% Parse optional parameters.解析可选参数
    ...
    % 问题：这里problem.opt_prob参数应该是还没有计算整个波导电磁场的参数，只有输入输出波导本征模式信息？
    ...
    % 此处仅定义了函数my_track_progress，并未开始调用
    ...
    %% Specify the parameterization指定参数化
    % This includes the structure design objective.
    
    % Helper辅助 functions needed for the level-set parameterization.
    function [u] = vec(u)
        u = u(:);
    end
