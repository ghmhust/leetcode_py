...
%% Source code
    ...
    % Also get doubled加倍 dimensions so we can upsample in 2D in order to take care
    % of the Yee grid offsets偏移.
    dims = size(epsilon{1});
    ...
    % Position data.
    x = (1:dd(1)) ./ 2; % 点数加倍，长度不变，不是从零开始，1*180
    y = (1:dd(2)) ./ 2; % 点数加倍，长度不变，不是从零开始,1*140
    [x, y] = ndgrid (x, y); %均为180*140

    % Find the center, shape positions are all relative to this point.
    % 这个二维平面的中心点是建立形状的相对点，所有位置不是以原点而是以这个点为基准
    center = [mean(mean(x)) mean(mean(y))]; % Naive天真的、实在的 center.
    center = floor(2 * round(center)/2) + [0.5 0]; % Slight offset for symmetry.
    % mean返回沿数组中不同维的元素的平均值，如果A是一个向量，mean(A)返回A中元素的平均值
